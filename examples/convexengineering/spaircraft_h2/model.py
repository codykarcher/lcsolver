...
from SP_hydrogen_aircraft.cryo_tank import add_cryo_tank  # noqa: E402
...
def build(Nclimb: int = NCLIMB, Ncruise: int = NCRUISE,
    ...
    *solver*, not the model: the constraints are verified independently by
    ...
    f_wingfuel = C("f_wingfuel", 1e-6, "-", "fraction of fuel in wing tanks")
    ...
    cons += [
        ...
        # ---- weight build-up -------------------------------------------------
        fu.W_fuse + numeng * Wengsys + fu.W_tail + wing.W_wing + Wmisc
            + tank["W_tank"] <= W_dry,
