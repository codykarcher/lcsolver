    ...
    *solver*, not the model: the constraints are verified independently by
    ...
    cons += [
        ...
        *([] if arch.rear_engines
        ...
        # PINNED (==), no longer a floor: the leg mounts ON the spar line at
        # its own spanwise station, not somewhere aft of it -- a floating
        # station was one more lever the CG chain could work. With the
        # default GEAR_BOX_FRAC = 1.00 this is the REAR SPAR at y_m, the
        # spar line approximated as the root box edge swept by tan_Lambda.
        lg.x_m == ((1.0 - _GEAR_BOX_FRAC) * fu.x_b + _GEAR_BOX_FRAC * fu.x_f
                   + lg.y_m * wing.tan_Lambda),   # [SP] SigEq
        #
