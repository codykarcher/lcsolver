"""v3's tabulated compressor maps -- against TASOPT.jl.

The 2.16 analytic ``ecmap`` lives in ``test_maps.py``; these are the
pyCycle tables that replace it. Both are kept -- the port supports
either engine model, and they are different physics, not two versions
of the same thing.
"""
import csv
import math
